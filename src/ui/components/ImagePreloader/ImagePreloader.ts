...
const urls = Object.keys(modules).map(it => it.replace("/public", ""));
const urlBatchSize = 20;
const startingDelay = 300;
...
export function ImagePreloader() {
    ...
    // ==== trigger all images with low fetchPriority after some delay ====
    // ==== with delay in between of image batches ====
    // const canRender = useBooleanState(false);
    const [urlsToRender, setUrlsToRender] = useState<string[]>([]);
    useEffect(() => {
        const startTime = Date.now();
        console.group("Starting image batch preloading");
